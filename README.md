...
## Latest News 🚀
* 08/10/2026 **API performance and safety update**: call-local and bounded Pattern fast paths now accelerate literal and capture-heavy `findall`, `split`, `sub`/`subn`, `Match.expand`, `lastindex`, flag dispatch, `template()`, and `escape`, with representative gains from **2x to 30x+** on Python 3.10 and free-threaded Python 3.14t/GIL=0. Compatibility fallbacks preserve complex-pattern, subclass, buffer, and callable behavior; caches and Pattern metadata remain bounded without retaining subjects or results. The release also closes unsafe UTF bytes compilation, backed by differential, randomized, concurrency, subprocess, and memory-safety tests. ⚡🛡️
