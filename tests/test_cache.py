...
def test_cache_strategy_benchmark(pytestconfig: pytest.Config) -> None:
    ...
    # Only benchmark thread counts supported by the host CPU to avoid measuring oversubscription noise.
    thread_counts = [count for count in desired_thread_counts if count == 1 or count <= cpu_count]
    scenarios = [
    ...
    for thread_count in thread_counts:
        ...
        # practical thread counts (avoid failing on informational high-concurrency measurements).
        if thread_count <= min(cpu_count, 8):
